...
void move_little() {
    int i; for ( i = 0; i != 200; i++ ) systick_wait1ms();
}
...
void turn_little() {
    int i; for ( i = 0; i != 500; i++ ) systick_wait1ms();
}
...
void main(void)
{
    ...
    while(1) {
        ...
        int detected_center = (status[3] == 1) || (status[4] == 1);
        ...
        */
        ...
        printf("\n");
        printf("left_feel %.2f, right_feel %.2f\n", left_feel, right_feel);
        printf("detected_left: %d, detected_right: %d, detected_center: %d\n\n", detected_left, detected_right, detected_center);
        //printf("%x l%d r%d\n", status, !!left, !!right);
        if ( !detected_center ) {
            if ( status[2] && status[5] ) {
                stop(); wait_little();
            }
            if ( status[2] ) { // detected_right
                left_backward(), right_backward(), move(10,10);
                move_little(); stop(); wait_little();
                left_forward(), right_backward(), move(10,10);
                turn_little(); stop(); wait_little();
            }
            else if ( status[5] ) {
                left_backward(), right_backward(), move(10,10);
                move_little(); stop(); wait_little();
            ...
            move_little(); stop(); wait_little();
            left_feel *= decay; left_feel += status[5] || status[6] || status[7];
            right_feel *= decay; right_feel += status[0] || status[1] || status[2];
        }
        /*
